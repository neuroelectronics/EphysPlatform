...
#define INTERCOM_CHECK_RXNE(__IC_handle__) (((__IC_handle__)->UART->SR&USART_SR_RXNE)!=0)
#define INTERCOM_CHECK_TXE(__IC_handle__) (((__IC_handle__)->UART->SR&USART_SR_TXE)!=0)
#define INTERCOM_RX_IS_ON(__IC_handle__) (((__IC_handle__)->state&INTERCOM_STATE_RECIVING)!=0)
#define INTERCOM_TX_SEND(__IC_handle__,__DATA__) while(((__IC_handle__)->UART->SR&USART_SR_TXE)==0);((__IC_handle__)->UART->DR=(__DATA__))
...
int CE32_INTERCOM_Incoming_CMD_Len(int index);																						//Returning correct command length
...
int CE32_INTERCOM_RX_ISR(CE32_INTERCOM_Handle *handle);
